"""Pipeline details templates based on the Tasks detail page layout.

The Tasks details page is the golden frontend CRUD template.  This module keeps
its shell, panel, form, message, toolbar, and button structure intact, then adds
only the Pipeline-specific description field required by the Pipeline entity.
"""
            ...
            <section class="protected-content-panel">
                ...
                <div class="ccore-pipeline-details-content">
                    ...
                    <header class="ccore-pipeline-details-header">
                        ...
                        <p>
                            Manage the pipeline definition, description, status, and persisted PostgreSQL CRUD lifecycle.
                        </p>
                    </header>
                        ...
                        <form id="ccorePipelineDetailsForm" class="ccore-pipeline-details-form">
                            ...
                            <div class="ccore-pipeline-details-grid">
                                ...
                                <label class="ccore-pipeline-details-field ccore-pipeline-description-field"
                                    ...
                                    <textarea id="ccorePipelineDescriptionInput" maxlength="2000" rows="4"></textarea>
                                </label>
 ...
 * - Keep the details page interaction model aligned with the Tasks module.
 */

const CCORE_PIPELINE_STATUSES_EMPTY_MESSAGE =
    "No pipeline statuses are configured.";

const CCORE_PIPELINE_DETAILS_ERROR_MESSAGE =
    "CCore pipeline details could not be loaded.";

const CCORE_PIPELINE_NAME_REQUIRED_MESSAGE =
    "Pipeline name is required.";

const CCORE_PIPELINE_STATUS_REQUIRED_MESSAGE =
    "Pipeline status is required.";

const CCORE_PIPELINE_NO_CHANGES_MESSAGE =
    "No changes to update.";

const CCORE_PIPELINE_CREATED_SUCCESS_MESSAGE =
    "Pipeline created successfully.";

const CCORE_PIPELINE_UPDATED_SUCCESS_MESSAGE =
    "Pipeline updated successfully.";

const CCORE_PIPELINE_SAVE_ERROR_MESSAGE =
    "Pipeline could not be saved.";

const CCORE_PIPELINE_DELETE_CONFIRM_MESSAGE =
    "Delete this CCore pipeline?";

const CCORE_PIPELINE_DELETE_ERROR_MESSAGE =
    "Pipeline could not be deleted.";
...
function setCCorePipelineDetailsMode(mode) {
    ...
    document.getElementById("ccorePipelineDetailsTitle").textContent =
        isEditMode ? "CCore Pipeline Details" : "Create CCore Pipeline";

    document.getElementById("ccorePipelineDetailsLegend").textContent =
        isEditMode ? "Pipeline Details" : "Create Pipeline";

    document.getElementById("ccorePipelineDetailsMode").textContent =
        isEditMode ? "Edit mode" : "Create mode";

    document.getElementById("saveCCorePipelineButton").textContent =
...
async function handleDeleteCCorePipelineClick() {
...
function setupCCorePipelineDetailsForm() {
    ...
    if (form) {
        form.addEventListener("submit", handleSaveCCorePipelineSubmit);
    }

    const deleteButton = document.getElementById("deleteCCorePipelineButton");

    if (deleteButton) {
        deleteButton.addEventListener("click", handleDeleteCCorePipelineClick);
    }
}
...
def build_detail_css(tasks_detail_css: str, entity: EntityConfig) -> str:
    css = tasks_detail_css
    replacements = [
...
.ccore-pipeline-details-field textarea {
