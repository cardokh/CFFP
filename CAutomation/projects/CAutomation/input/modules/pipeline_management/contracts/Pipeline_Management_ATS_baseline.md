...
**Document status:** Baseline production contract for the Pipeline Management reference project. Only this ATS and the matching SRS are manually authored and approved at module level. This baseline hardens table names, field expectations, API shapes, frontend screens, repository targets, validation envelopes, execution orchestration, runtime configuration merge behavior, and traceability requirements for deterministic generation.
...
| pipeline_execution | Execution instance for a pipeline version. | id, pipeline_id, pipeline_version, execution_key, status, started_at, completed_at, requested_by, execution_parameters_json, summary_message |
...
| PipelineExecutionDto | Expose id, pipeline id, version, execution_key, status, timestamps, requested_by, execution_parameters_json, and summary. |
...
| PipelineExecutionService | Create execution snapshots, transition statuses through native FastAPI BackgroundTasks, enforce runtime parameter merge behavior, and expose execution detail. |
| ReportingService | Create and retrieve reports linked to execution or task execution. |
| MetadataService | Provide read operations for all active lookup tables to authenticated users, and restrict mutate operations to the System Administrator role as defined in the Project Client Contract. |
...
| SRS-VAL-011 | Runtime execution parameter merge validator shall shallow-merge execution_parameters_json into the immutable baseline pipeline_task.configuration_json snapshot, shall apply runtime duplicate-key override semantics, and shall persist the resulting merged configuration as immutable for the lifetime of the Pipeline Execution. |
...
| Service tests | Validation rules, lifecycle transitions, transactional behavior, context validation, task reuse, runtime execution parameter merge behavior, and parent-boundary enforcement. |
...
- Generated tests cover positive flows, validation failures, lifecycle transitions, task reuse, execution snapshots, runtime execution parameter merge behavior, and report generation.
...
| pipeline_execution | execution_parameters_json | Runtime execution parameter overrides | Nullable JSON. The execution snapshot shall be created at runtime by performing a shallow merge of execution_parameters_json onto the baseline pipeline_task.configuration_json record. Runtime keys must override matching base configuration keys. The resulting merged configuration snapshot shall become immutable for the remaining lifetime of the Pipeline Execution record. |
...
| execution_report | content_path | Report content path | Required when report body stored externally. Storage shall be strictly constrained inside the approved `.cautomation_workspace` staging or target boundaries in compliance with the Safe path rule defined in the Project Engineering Contract. No alternative storage layout, external cloud bucket, or unmapped file location may be inferred by the generation engine. |
...
| Start execution | Pipeline execution and task execution rows must commit together. | Execution snapshots the pipeline version, PipelineTask order, reusable task keys, and runtime merged configuration used at start time. |
...
| Execution lifecycle | pipeline_execution and pipeline_task_execution tables and services. | Execution creation, status transition, runtime merge, and immutable execution tests. |
...
| GRP-011 | Runtime execution configuration is deterministic. | execution_parameters_json shallow-merge behavior, duplicate-key override semantics, and immutable merged snapshot rules are present in SRS and ATS. |
| GRP-012 | Execution orchestration is deterministic. | Native FastAPI BackgroundTasks is mandated as the exclusive backend execution orchestration mechanism. |
...
- Do not generate or apply execution code that uses orchestration mechanisms other than native FastAPI BackgroundTasks.
- Do not generate or apply report storage paths outside the approved `.cautomation_workspace` boundaries.
