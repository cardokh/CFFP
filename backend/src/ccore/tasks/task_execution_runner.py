...
def inspect_automation_factory_contracts_step(project_root: str) -> dict:
    ...
    if not runner_file.is_file():
        ...
        checks.append(
            _build_check(
                ...
                STATUS_PASSED if "GenerateDocumentationTaskRunner" in runner_text else STATUS_FAILED,
    ...
    if not constants_file.is_file():
        ...
        checks.append(
            _build_check(
                "CCORE_TASK_RUNNER_GENERATE_DOCUMENTATION",
                STATUS_PASSED if "CCORE_TASK_RUNNER_GENERATE_DOCUMENTATION" in constants_text else STATUS_FAILED,
...
def generate_automation_factory_documentation_step(project_root: str) -> dict:
    root = Path(project_root).resolve()
    documentation_path = root / "docs" / "automation_factory" / "automation_factory_capabilities.md"
...
def _load_seeded_task_names(root: Path) -> list[str]:
    seed_path = root / "scripts" / "db" / "postgres" / "config" / "postgres_seed_data.json"
    if not seed_path.is_file():
        return [VALIDATE_PROJECT_TASK_NAME, INSPECT_PROJECT_TASK_NAME, GENERATE_DOCUMENTATION_TASK_NAME]

    seed_data = json.loads(seed_path.read_text(encoding="utf-8"))
    for seed_group in seed_data.get("seedGroups", []):
        if seed_group.get("table") == "ccore_tasks":
            return [row["task_name"] for row in seed_group.get("rows", []) if "task_name" in row]
...
class CCoreTaskRunnerRegistry:
    def __init__(self, project_root):
        ...
        self.runners_by_task_name = {
            ...
            GENERATE_DOCUMENTATION_TASK_NAME: GenerateDocumentationTaskRunner(self.project_root),
        }
